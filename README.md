...
各 run 模型骨干相同（chunk-based local DiT，motion dim 384、K=16、EMA），差异在特征流 / 监督目标 / 数据集版本。5 个新 run（v3/v4）散点样本一致：N=323（episodes 90–99）。v4 两个光流 run 已训练完成（epoch 40）。5 个 v3/v4 run 全部 epoch 40，仅既有 v2 dual 为 epoch 16。入口页 [index.html](https://hongzefu.github.io/motionjepa-viz/) 提供对比导航与差异表。
