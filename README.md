...
| [localDiT_4env_v4_flowSliding epoch 4](https://hongzefu.github.io/motionjepa-viz/localDiT_4env_v4_flowSliding_epoch4/interactive.html) | 三流 +光流(sliding, w=1.0) | `checkpoint_epoch_4.pt` (EMA, 训练中) | v4 |
| [localDiT_4env_v4_flowSum epoch 4](https://hongzefu.github.io/motionjepa-viz/localDiT_4env_v4_flowSum_epoch4/interactive.html) | 三流 +光流(sum, w=0.4 降权) | `checkpoint_epoch_4.pt` (EMA, 训练中) | v4 |
...
各 run 模型骨干相同（chunk-based local DiT，motion dim 384、K=16、EMA），差异在特征流 / 监督目标 / 数据集版本。5 个新 run（v3/v4）散点样本一致：N=323（episodes 90–99）。v4 两个光流 run 仍在训练，挂的是 epoch 4 早期快照。入口页 [index.html](https://hongzefu.github.io/motionjepa-viz/) 提供对比导航与差异表。
